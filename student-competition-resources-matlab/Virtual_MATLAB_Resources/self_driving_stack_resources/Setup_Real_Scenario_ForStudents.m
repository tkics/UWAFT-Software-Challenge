...
myStopSign.spawn_degrees([1.523, 1.232, 0.006], ...
                        [0, 0, 90], ...
                        [0.1, 0.1, 0.1], ...
                        false);
...
%% ===== Spawn 1 Pedestrian (back-and-forth) =====
import qvl.person.*

PERSON_SCALE = [0.1 0.1 0.1];         % tune
WALK_SPEED   = QLabsPerson.WALK * 0.2;   % tune

p1 = QLabsPerson(qlabs, verbose);

% Endpoints (as you showed)
p1_A = [-0.350, 1.559, 0.004];
p1_B = [ 0.650, 1.559, 0.004];

% Spawn at A
p1.spawn_degrees(p1_A, [0 0 0], PERSON_SCALE, 0);

% Start walking A -> B
p1.move_to(p1_B, WALK_SPEED, true);

% Timer: flip destination every PED_PERIOD seconds
PED_PERIOD = 4.0;   % seconds per leg (A->B or B->A). Full cycle is 2*PED_PERIOD.
...
function pedStep(tmr, ~)
    ud = tmr.UserData;
    ud.toggle = ~ud.toggle;

    if ud.toggle
        ud.p1.move_to(ud.p1_A, ud.speed, true);
    else
        ud.p1.move_to(ud.p1_B, ud.speed, true);
    end
