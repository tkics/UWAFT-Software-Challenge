...
try:
    ...
    probe.add_plot(numMeasurements=1680, scaling=True,
                      scalingFactor= 4, name='Leishen Lidar')
    startTime = time.time()
    time.sleep(0.5)
    ...
    while noKill and not endFlag:
        ...
        if probe.connected:
            ...
            if newHIL:
                ...
                newRealSenseDepth = realSenseCam.read_depth(dataMode='PX')
                ...
                if counterRS%4 == 2:
                    sending = probe.send(name="RealSense Depth Image",
                                         imageData=cv2.convertScaleAbs(realSenseCam.imageBufferDepthPX, alpha=(255.0/(255.0*255.0))))
                elif counterRS%4 == 0:
